...
// SA Integration
#define FUNC_DrawTexturedRect				0x728350
#define FUNC_CleanTexture					0x727240
#define FUNC_LoadTexture					0x7272B0
#define FUNC_ReloadTexture					0x727270

// Doesn't fit?
#define FUNC_ReadTexture					0x7F3AC0

// TODO: Move
struct RwRect2D{float x1,y1,x2,y2;};
struct RwRGBA{BYTE r,g,b,a;};
...
typedef class RwV2d RwV2d;
typedef class RwV3d RwV3d;
...
class RwV2d		// sizeof=8
{
public:
    float x,y;
};
typedef RwV2d RwV2D;
class RwV3d		// sizeof=12
{
public:
    float x,y,z;

	RwV3d():											x(0),y(0),z(0)			{};
	RwV3d(float a, float b, float c):					x(a),y(b),z(c)			{};
	RwV3d(const RwV3d &v):								x(v.x),y(v.y),z(v.z)	{};
	RwV3d& operator = (const RwV3d &v)
	{
		x = v.x; y = v.y; z = v.z;
		return *this;
	}
	...
	RwV3d operator + (const RwV3d &v)							{return RwV3d(x+v.x,y+v.y,z+v.z);}
	RwV3d operator - (const RwV3d &v)							{return RwV3d(x-v.x,y-v.y,z-v.z);}
	RwV3d operator * (float v) const							{return RwV3d(x*v,y*v,z*v);}

	RwV3d operator - () const									{return RwV3d(-x,-y,-z);}

	friend bool operator > (const RwV3d &v, const RwV3d &w)		{return(v.x>w.x&&v.y>w.y&&v.z>w.z);}
	friend bool operator < (const RwV3d &v, const RwV3d &w)		{return(v.x<w.x&&v.y<w.y&&v.z<w.z);}
	friend bool operator >= (const RwV3d &v, const RwV3d &w)	{return(v.x>=w.x&&v.y>=w.y&&v.z>=w.z);}
	friend bool operator <= (const RwV3d &v, const RwV3d &w)	{return(v.x<=w.x&&v.y<=w.y&&v.z<=w.z);}
	friend bool operator == (const RwV3d &v, const RwV3d &w)	{return(v.x==w.x&&v.y==w.y&&v.z==w.z);}
...
struct RwTexture
{
	...
	static void			Draw(RwTexture**,RwRect2D*,RwRGBA*);
	static void			Load(RwTexture**,char *name,char *maskName);
	static void			Reload(RwTexture**,char *name);
	static void			Clean(RwTexture**);
	static RwTexture  *	Read(char *name,char *maskName);
};
